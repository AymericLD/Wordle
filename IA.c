...
char* fusion(char c[5]) {
	char* f = malloc(sizeof(char*)*5);
	for(int i=0;i<=4;i++) {
		f[i] = c[i];
	}
	return f;
...
void possibilites(char** possibles) {
	...
	for(int n=0;n<=242;n++) { //passage en écriture ternaire
		...
		combi[n] = fusion(cb); // le k-ème élément de combi contient une combinaison unique
	}
	possibles = combi;
...
bool lettres_interdites(char* combi, char* mot_info, char* mot_dico) { //nous dit si un mot contient une lettre interdite
	...
	for(k=0;k<=4;k++) {
		for(int j=0;j<=4;j++) {
			if(combi[k] == '*' && mot_info[j] == mot_dico[k] && occ(mot_dico[k],mot_info) == 1) { //nombre d'occurence !
				return true;
			}
		}
	} return false;
...
struct information information_mot(char** ensemble_mots, char* mot) { //renvoie la structure d'information d'UN SEUL MOT dans le dictionnaire
	...
	char** combinaisons = malloc(243*sizeof(char*));
    ...
    double* pos = malloc(sizeof(double*)*243); //sizeof(double) ou sizeof(double*) ?
    ...
    for(int i=0;i<=242;i++) { //Pour un i fixe on va regarder l'info associée
    	...
    	for(int j=0;j<=size;j++) { //LES ELEMENTS DU DICO SONT SUPPOSES DEJA DE LA BONNE TAILLE
    		...
    		if(lettres_interdites(une_combi,mot,mot_dico) == false && lettres_misplace(une_combi,mot,mot_dico) == false) { //rajouter éventuellement des tests
    			compteur++;
    		}
...
char* max_entropie(char** ensemble_mots) {
	...
	struct entropies* liste_entrop = malloc(sizeof(struct entropies*)); //* ou non ?
	...
	char* bon_mot; //on va supposer l'unicité des entropies ...
...
int main() {
    ...
    char** L=malloc((longueur)*sizeof(char*));

	char* mot_cherche = mot_aleatoire(longueur,"dico.txt",L,5); //mot que l'IA doit trouver
